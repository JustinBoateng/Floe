...
public class PlayerControl : MonoBehaviour
{
    ...
    private void Movement()
    {
        ...
        WallJumpInfo[1] += Time.deltaTime;
        if (WallJumpInfo[1] >= WallJumpInfo[0])
        {
            rb.gravityScale = 1;
    ...
    */
    ...
    private void WallSlide()
    {
        if(isOnWall() && !isGrounded() && horizontalInput == facing[0])
        {
            ...
            //clamp makes it so that the min is rb.velocity.y and the max is -wallJumpInfo[4]. this makes it so the rb.velocity.y can be no less than -WallJumpInfo[4]
        ...
        if (context.started) 
        {
            if (WallJumpInfo[1] >= WallJumpInfo[0])
            {
                ...
                else if (isOnWall() && !isGrounded())
                {
                    ...
                    transform.localScale = new Vector3(-Mathf.Sign(facing[0]), transform.localScale.y, transform.localScale.z);
                    WallJumpInfo[1] = 0;
